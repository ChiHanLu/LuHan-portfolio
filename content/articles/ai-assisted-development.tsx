...
export default function Article() {
  return (
    <>
      ...
      <Lead>老實說，一開始我是非常排斥 AI 的。總覺得用 AI 寫程式就像是作弊一樣，不是「真正」的程式設計能力。</Lead>
      <P>但當我看到身邊的同學都在使用 AI，而且他們的學習速度明顯比我快時，我開始重新思考這件事。</P>
      <P>今天想分享我從完全排斥到學會善用 AI 的完整轉變過程。</P>
      ...
      <P>但看著身邊同學們用 AI 輔助學習，效率明顯比我高，我開始懷疑自己的堅持是否正確。</P>

      <H2 id="sec-3">第二階段：嘗試免費版本</H2>
      <H3>小心翼翼的第一步</H3>
      <P>在同學的建議下，我終於決定試試看免費版的 ChatGPT。</P>
      <P><strong>第一次使用的心境</strong>：</P>
      <Ul>
        <Li>🤖 問了一個超簡單的問題：「如何在 Python 中讀取檔案？」</Li>
        <Li>😮 AI 的回答比我想像中詳細</Li>
        <Li>🤷‍♂️ 但老實說，當下還是覺得「我自己查資料也能找到」</Li>
      </Ul>
      <P>那時候的我還沒意識到，AI 的價值不只是回答問題，而是能夠理解上下文，提供個人化的解答。</P>

      <H2 id="sec-4">第三階段：害怕與不熟悉</H2>
      <H3>擔心 AI 給出我看不懂的東西</H3>
      <P>開始使用 AI 後，我遇到了一個新問題：AI 有時候會給出很複雜的程式碼，我根本看不懂！</P>
      <P><strong>當時的困擾</strong>：</P>
      <Ul>
        <Li>😰 AI 給的程式碼有很多我沒學過的語法</Li>
        <Li>🤯 不知道該直接用還是先學會再用</Li>
        <Li>😓 害怕用了不懂的程式碼會出問題</Li>
        <Li>🤔 開始懷疑是不是我的問題描述不夠清楚</Li>
      </Ul>
      <P><strong>轉捩點</strong>：有一次 AI 給了我一段 Flask 路由的程式碼，裡面用了裝飾器。我完全不懂裝飾器是什麼，但我決定花時間去理解它，而不是直接複製貼上。</P>

      <H2 id="sec-5">第四階段：從複製貼上到理解學習</H2>
      <H3>學會了「審閱」AI 的回答</H3>
      <P>這個階段是我最重要的轉變！我開始養成一個習慣：</P>
      ...
      <P>AI 給了我一段用戶註冊的程式碼，但我發現它沒有考慮到我專案的特殊需求（例如需要驗證學校信箱），所以我會在理解原理後，加入自己的邏輯。</P>
      ...
      <Ul>
        <Li>✅ 每一行程式碼都要理解</Li>
        <Li>✅ 不懂的概念立刻查資料或問 AI</Li>
        <Li>✅ 嘗試用自己的話解釋程式碼在做什麼</Li>
        <Li>❌ 不要因為「能跑」就不管原理</Li>
      </Ul>
      ...
      <Ul>
        <Li>🚀 <strong>敢問複雜問題</strong>：不要因為問題太難就不問</Li>
        <Li>🔄 <strong>多次迭代優化</strong>：第一次的回答不滿意就繼續問</Li>
        <Li>🧪 <strong>實際測試</strong>：把 AI 的建議實際跑在專案上</Li>
...
現在：「我正在用 Flask 開發日記應用，需要實作用戶登入功能。
      ...
      <H3>從排斥到依賴的心境變化</H3>
      <P>回顧這段歷程，我發現自己的心境有了 180 度的轉變：</P>
      ...
      <P>從排斥到依賴，這個轉變讓我深刻體會到：<strong>適應新工具比固守舊方法更重要</strong>。</P>
